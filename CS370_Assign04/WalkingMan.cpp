...
void build_materials( ) {
    ...
    MaterialProperties redAcrylic = {
            vec4(0.3f, 0.0f, 0.0f, 0.8f), //ambient
            vec4(0.6f, 0.0f, 0.0f, 0.8f), //diffuse
            vec4(0.8f, 0.6f, 0.6f, 0.8f), //specular
            32.0f, //shininess
            {0.0f, 0.0f, 0.0f}  //pad
    };
...
void build_scene_graph( ) {
    ...
    torso.set_update_transform(translate(vec3(0.0f, LOWER_LEG_HEIGHT+UPPER_LEG_HEIGHT, 0.0f)));
    ...
    left_lower_arm.set_update_transform(translate(0.0f, -UPPER_ARM_HEIGHT, 0.0f));
    ...
    right_lower_arm.set_update_transform(translate(0.0f, -UPPER_ARM_HEIGHT, 0.0f));
    right_lower_arm.attach_nodes(NULL, NULL);

    build_lighting_node(left_upper_leg, Cube, GreenJersey, false, scale(UPPER_LEG_WIDTH, UPPER_LEG_HEIGHT, UPPER_LEG_DEPTH));
    left_upper_leg.set_update_transform(translate((-UPPER_LEG_WIDTH+0.25f), TORSO_HEIGHT/2 - UPPER_LEG_HEIGHT/2, 0.0f));
    left_upper_leg.attach_nodes(&left_lower_leg, &right_upper_leg);

    build_lighting_node(right_upper_leg, Cube, GreenJersey, false, scale(UPPER_LEG_WIDTH, UPPER_LEG_HEIGHT, UPPER_LEG_DEPTH));
    right_upper_leg.set_update_transform(translate((UPPER_LEG_WIDTH-0.25f), TORSO_HEIGHT/2 - UPPER_LEG_HEIGHT/2, 0.0f));
    right_upper_leg.attach_nodes(&right_lower_leg, NULL);

    build_lighting_node(left_lower_leg, Cube, Skin, false, scale(LOWER_LEG_WIDTH, LOWER_LEG_HEIGHT, LOWER_LEG_DEPTH));
    left_lower_leg.set_update_transform(translate(0.0f, -UPPER_LEG_HEIGHT, 0.0f));
    left_lower_leg.attach_nodes(NULL, NULL);

    build_lighting_node(right_lower_leg, Cube, Skin, false, scale(LOWER_LEG_WIDTH, LOWER_LEG_HEIGHT, LOWER_LEG_DEPTH));
    right_lower_leg.set_update_transform(translate(0.0f, -UPPER_LEG_HEIGHT, 0.0f));
    right_lower_leg.attach_nodes(NULL, NULL);

    build_texture_node(court, TexCube, Wood, scale(15.0f, 0.1f, 15.0f));
    court.set_update_transform(translate(0.0f, -LOWER_LEG_HEIGHT, 0.0f));
    court.attach_nodes(NULL, &box_node);

    build_lighting_node(box_node, Cube, RedAcrylic, true, scale(BOX_WIDTH, BOX_HEIGHT, BOX_DEPTH));
    box_node.set_update_transform(translate(3.0f, -LOWER_LEG_HEIGHT+BALL_RADIUS, 3.0f));
    box_node.attach_nodes(NULL, &ball);

    build_texture_node(ball, Sphere, Basketball, scale(BALL_RADIUS, BALL_RADIUS, BALL_RADIUS));
    ball.set_update_transform(translate(3.0f, -LOWER_LEG_HEIGHT+BALL_RADIUS, 3.0f));
    ball.attach_nodes(NULL, &left_upper_leg);
}
