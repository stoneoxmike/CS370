...
enum GolfNorms {FLAT, OUT, IN, NumGolfNorms};
...
// Model files
vector<const char *> objFiles = {"../models/unitcube.obj", "../models/uv_sphere.obj", "../models/torus.obj"};

// Texture files
vector<const char *> texFiles = {"../textures/carpet.jpg", "../textures/dirt.png", "../textures/blank.png", "../textures/golf4.png", "../textures/golfNormFlat.png", "../textures/golfNormOut.png", "../textures/golfNormIn.png"};
...
int main(int argc, char**argv)
{
	...
	GLFWwindow* window = CreateWindow("Bump Mesh");
    if (!window) {
...
void display( )
{
    ...
    proj_matrix = frustum(-0.5f*xratio, 0.5f*xratio, -0.5f*yratio, 0.5f*yratio, 0.5f, 100.0f);
...
void render_scene( ) {
    ...
    if (dirty) {
        // Use Carpet and Dirt textures
        draw_multi_tex_object(Cube, Carpet, Dirt);
    } else {
        // Use Carpet and Blank textures
    ...
    if(!bump) {
        // Render sphere with multi-texturing
        if (dirty) {
            // Use Golf and Dirt textures
            draw_multi_tex_object(UVSphere, Golf, Dirt);
        } else {
            // Use Golf and Blank textures
            draw_multi_tex_object(UVSphere, Golf, Blank);
        }
    } else {
        // Render sphere with bumpmapping
        // Compute normal matrix from model matrix
        normal_matrix = model_matrix.inverse().transpose();

        if (golf == FLAT) {
            // Use Golf and flat normal map
            draw_bump_object(UVSphere, Golf, GolfNormFlat);
        } else if (golf == OUT) {
            // Use Golf and outward normal map
            draw_bump_object(UVSphere, Golf, GolfNormOut);
        } else if (golf == IN) {
            // Use Golf and inward normal map
            draw_bump_object(UVSphere, Golf, GolfNormIn);
        }
    }
}
...
void build_geometry( )
{
    ...
    // Load bump mapped objects (tangents)
    load_bump_object(UVSphere);
}
