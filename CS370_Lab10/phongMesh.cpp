...
void draw_phong_mat_object(GLuint obj, GLuint material);
...
void render_scene( ) {
    ...
    // Draw object
    draw_phong_mat_object(Cube, Brass);
    ...
    normal_matrix = model_matrix.inverse().transpose();
    // Draw object
    draw_phong_mat_object(Sphere, RedPlastic);
}
...
void draw_phong_mat_object(GLuint obj, GLuint material){
    ...
    glUniformMatrix4fv(phong_proj_mat_loc, 1, GL_FALSE, proj_matrix);
    glUniformMatrix4fv(phong_camera_mat_loc, 1, GL_FALSE, camera_matrix);

    // Bind lights
    glUniformBlockBinding(phong_program, phong_lights_block_idx, 0);
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, LightBuffers[LightBuffer], 0, Lights.size()*sizeof(LightProperties));

    // Bind materials
    glUniformBlockBinding(phong_program, phong_materials_block_idx, 1);
    glBindBufferRange(GL_UNIFORM_BUFFER, 1, MaterialBuffers[MaterialBuffer], 0, Materials.size()*sizeof(MaterialProperties));

    // Set camera position
    glUniform3fv(phong_eye_loc, 1, eye);

    // Set num lights and lightOn
    glUniform1i(phong_num_lights_loc, numLights);
    glUniform1iv(phong_light_on_loc, numLights, lightOn);

    // Pass model matrix and normal matrix to shader
    glUniformMatrix4fv(phong_model_mat_loc, 1, GL_FALSE, model_matrix);
    glUniformMatrix4fv(phong_norm_mat_loc, 1, GL_FALSE, normal_matrix);

    // Pass material index to shader
    glUniform1i(phong_material_loc, material);
    ...
    glBindBuffer(GL_ARRAY_BUFFER, ObjBuffers[obj][PosBuffer]);
    glVertexAttribPointer(phong_vPos, posCoords, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(phong_vPos);

    // Bind normal object buffer and set attributes
    glBindBuffer(GL_ARRAY_BUFFER, ObjBuffers[obj][NormBuffer]);
    glVertexAttribPointer(phong_vNorm, normCoords, GL_FLOAT, GL_FALSE, 0, NULL);
    glEnableVertexAttribArray(phong_vNorm);
